...
app.get("/api/spin-config", async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error al cargar la configuración de la ruleta:",
      error.message
    );
...
app.post("/api/register", async (req, res) => {
  ...
  if (!plate || !email || !phone) {
    return res
      .status(400)
      .json({ message: "Placa, email y teléfono son requeridos." });
  }
...
app.post("/api/spin", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Usuario ${plate} tiene ${user.spinsAvailable} giros restantes.`
    );
    ...
    if (prize.text.toLowerCase() === "un giro adicional") {
      ...
      if (!existingPrize) {
        ...
        console.log(
          `Premio ya existente para el usuario ${plate}: ${prize.text}`
        );
      }
    }
    ...
    console.log(
      `Prize Index: ${prizeIndex}, Prize Center Angle: ${prizeCenterAngle}°`
    );
    console.log(
      `Stop Angle Calculado para el premio "${prize.text}": ${stopAngle} grados`
    );
...
app.get("/api/health", async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error en la conexión a MongoDB en ruta de salud:",
      error.message
    );
